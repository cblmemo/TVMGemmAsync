...
class Module:
    ...
    def main(A: T.Buffer((512, 3072), "float16"), B: T.Buffer((3072, 768), "float16"), Y: T.Buffer((512, 768), "float16")):
        ...
        # with T.block("root"):
        Y_reindex_m16n8k8_matrixC = T.alloc_buffer((512, 768), "float16", scope="m16n8k8.matrixC")
        ...
        A_reindex_shared_dyn_m16n8k8_matrixA = T.alloc_buffer((512, 3072), "float16", scope="m16n8k8.matrixA")
        B_reindex_shared_dyn_m16n8k8_matrixB = T.alloc_buffer((3072, 768), "float16", scope="m16n8k8.matrixB")
        for ax0_0_0_ax1_0_0_fused in T.thread_binding(12, thread="blockIdx.x"):
            for ax0_0_1_ax1_0_1_fused in T.thread_binding(4, thread="blockIdx.y"):
                for ax0_0_2_ax1_0_2_fused in T.thread_binding(4, thread="threadIdx.y"):
                    for ax0_0_3_init, ax1_0_3_init, ax0_0_4_init, ax1_0_4_init in T.grid(4, 2, 1, 2):
                        with T.block("Y_o_init"):
                            v0_o = T.axis.spatial(32, ax0_0_0_ax1_0_0_fused // 3 * 8 + ax0_0_1_ax1_0_1_fused // 2 * 4 + ax0_0_3_init + ax0_0_4_init)
                            v1_o = T.axis.spatial(96, ax0_0_0_ax1_0_0_fused % 3 * 32 + ax0_0_1_ax1_0_1_fused % 2 * 16 + ax0_0_2_ax1_0_2_fused * 4 + ax1_0_3_init * 2 + ax1_0_4_init)
                            T.reads()
                    ...
                    for ax2_0_0 in T.serial(96, annotations={"software_pipeline_async_stages": [0], "software_pipeline_order": [0, 1, 3, 2, 4], "software_pipeline_stage": [0, 0, 1, 2, 2]}):
                        with T.block("A_reindex_shared.dyn"):
                            ...
                            T.reads(A[v0 // 3 * 128 + v1 // 2 * 64:v0 // 3 * 128 + v1 // 2 * 64 + 64, v2 * 32:v2 * 32 + 32])
                            T.writes(A_reindex_shared_dyn[v0 // 3 * 128 + v1 // 2 * 64:v0 // 3 * 128 + v1 // 2 * 64 + 64, v2 * 32:v2 * 32 + 32])
                            T.block_attr({"auto_copy": 1, "buffer_dim_align": [[0, 0, 32, 8]], "permuted_layout": "g2s_A", "vector_bytes": 16})
                            for ax0, ax1 in T.grid(64, 32):
                                A_reindex_shared_dyn[v0 // 3 * 128 + v1 // 2 * 64 + ax0, v2 * 32 + ax1] = A[v0 // 3 * 128 + v1 // 2 * 64 + ax0, v2 * 32 + ax1]
                        with T.block("B_reindex_shared.dyn"):
                            v0, v1, v2 = T.axis.remap("SSS", [ax2_0_0, ax0_0_0_ax1_0_0_fused, ax0_0_1_ax1_0_1_fused])
                            T.reads(B[v0 * 32:v0 * 32 + 32, v1 % 3 * 256 + v2 % 2 * 128:v1 % 3 * 256 + v2 % 2 * 128 + 128])
                            T.writes(B_reindex_shared_dyn[v0 * 32:v0 * 32 + 32, v1 % 3 * 256 + v2 % 2 * 128:v1 % 3 * 256 + v2 % 2 * 128 + 128])
                            T.block_attr({"auto_copy": 1, "buffer_dim_align": [[0, 0, 32, 8]], "permuted_layout": "g2s_B", "vector_bytes": 16})
                            for ax0, ax1 in T.grid(32, 128):
                                B_reindex_shared_dyn[v0 * 32 + ax0, v1 % 3 * 256 + v2 % 2 * 128 + ax1] = B[v0 * 32 + ax0, v1 % 3 * 256 + v2 % 2 * 128 + ax1]
                        for ax2_0_1 in T.serial(2, annotations={"software_pipeline_order": [0, 1, 2], "software_pipeline_stage": [0, 0, 1]}):
                            for ax0_0, ax1_0 in T.grid(2, 2):
                                with T.block("A_reindex_shared.dyn_m16n8k8.matrixA_o"):
                                    v0_o = T.axis.spatial(16, ax0_0_0_ax1_0_0_fused // 3 * 4 + ax0_0_1_ax1_0_1_fused // 2 * 2 + ax0_0)
                                    v1_o = T.axis.spatial(384, ax2_0_0 * 4 + ax2_0_1 * 2 + ax1_0)
                            ...
                            for ax0_0, ax1_0 in T.grid(2, 1):
                                with T.block("B_reindex_shared.dyn_m16n8k8.matrixB_o"):
                                    v0_o = T.axis.spatial(384, ax2_0_0 * 4 + ax2_0_1 * 2 + ax0_0)
                                    v1_o = T.axis.spatial(24, ax0_0_0_ax1_0_0_fused % 3 * 8 + ax0_0_1_ax1_0_1_fused % 2 * 4 + ax0_0_2_ax1_0_2_fused + ax1_0)
                            ...
                            for ax0_0_3, ax1_0_3, ax2_0_2, ax0_0_4, ax1_0_4 in T.grid(4, 2, 2, 1, 2):
                                with T.block("Y_o_update"):
                                    v0_o = T.axis.spatial(32, ax0_0_0_ax1_0_0_fused // 3 * 8 + ax0_0_1_ax1_0_1_fused // 2 * 4 + ax0_0_3 + ax0_0_4)
                                    v1_o = T.axis.spatial(96, ax0_0_0_ax1_0_0_fused % 3 * 32 + ax0_0_1_ax1_0_1_fused % 2 * 16 + ax0_0_2_ax1_0_2_fused * 4 + ax1_0_3 * 2 + ax1_0_4)
                                    v2_o = T.axis.reduce(384, ax2_0_0 * 4 + ax2_0_1 * 2 + ax2_0_2)
                    ...
                    with T.block("Y_reindex_m16n8k8.matrixC"):
                        ...
                        T.reads(Y_reindex_m16n8k8_matrixC[v0 // 3 * 128 + v1 // 2 * 64:v0 // 3 * 128 + v1 // 2 * 64 + 64, v0 % 3 * 256 + v1 % 2 * 128 + v2 * 32:v0 % 3 * 256 + v1 % 2 * 128 + v2 * 32 + 32])
                        T.writes(Y[v0 // 3 * 128 + v1 // 2 * 64:v0 // 3 * 128 + v1 // 2 * 64 + 64, v0 % 3 * 256 + v1 % 2 * 128 + v2 * 32:v0 % 3 * 256 + v1 % 2 * 128 + v2 * 32 + 32])
                        T.block_attr({"auto_copy": 1})
                        for ax0, ax1 in T.grid(64, 32):
                            Y[v0 // 3 * 128 + v1 // 2 * 64 + ax0, v0 % 3 * 256 + v1 % 2 * 128 + v2 * 32 + ax1] = Y_reindex_m16n8k8_matrixC[v0 // 3 * 128 + v1 // 2 * 64 + ax0, v0 % 3 * 256 + v1 % 2 * 128 + v2 * 32 + ax1]
