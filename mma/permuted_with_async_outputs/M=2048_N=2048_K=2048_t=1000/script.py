...
class Module:
    ...
    def main(A: T.Buffer((2048, 2048), "float16"), B: T.Buffer((2048, 2048), "float16"), Y: T.Buffer((2048, 2048), "float16")):
        ...
        # with T.block("root"):
        Y_reindex_m16n8k8_matrixC = T.alloc_buffer((2048, 2048), "float16", scope="m16n8k8.matrixC")
        ...
        A_reindex_shared_dyn_m16n8k8_matrixA = T.alloc_buffer((2048, 2048), "float16", scope="m16n8k8.matrixA")
        B_reindex_shared_dyn_m16n8k8_matrixB = T.alloc_buffer((2048, 2048), "float16", scope="m16n8k8.matrixB")
        for ax0_0_0_ax1_0_0_fused in T.thread_binding(32, thread="blockIdx.x"):
            for ax0_0_1_ax1_0_1_fused in T.thread_binding(4, thread="blockIdx.y"):
                for ax0_0_2_ax1_0_2_fused in T.thread_binding(4, thread="threadIdx.y"):
                    for ax0_0_3_init, ax1_0_3_init, ax0_0_4_init, ax1_0_4_init in T.grid(1, 16, 4, 1):
                        with T.block("Y_o_init"):
                            v0_o = T.axis.spatial(128, ax0_0_0_ax1_0_0_fused // 2 * 8 + ax0_0_2_ax1_0_2_fused // 2 * 4 + ax0_0_3_init * 4 + ax0_0_4_init)
                            v1_o = T.axis.spatial(256, ax0_0_0_ax1_0_0_fused % 2 * 128 + ax0_0_1_ax1_0_1_fused * 32 + ax0_0_2_ax1_0_2_fused % 2 * 16 + ax1_0_3_init + ax1_0_4_init)
                            T.reads()
                    ...
                    for ax2_0_0 in T.serial(128, annotations={"software_pipeline_async_stages": [0], "software_pipeline_order": [0, 1, 3, 2, 4], "software_pipeline_stage": [0, 0, 1, 2, 2]}):
                        with T.block("A_reindex_shared.dyn"):
                            v0, v1 = T.axis.remap("SS", [ax0_0_0_ax1_0_0_fused, ax2_0_0])
                            T.reads(A[v0 // 2 * 128:v0 // 2 * 128 + 128, v1 * 16:v1 * 16 + 16])
                            T.writes(A_reindex_shared_dyn[v0 // 2 * 128:v0 // 2 * 128 + 128, v1 * 16:v1 * 16 + 16])
                            T.block_attr({"auto_copy": 1, "buffer_dim_align": [[0, 0, 32, 8]], "permuted_layout": "g2s_A", "vector_bytes": 16})
                            for ax0, ax1 in T.grid(128, 16):
                                A_reindex_shared_dyn[v0 // 2 * 128 + ax0, v1 * 16 + ax1] = A[v0 // 2 * 128 + ax0, v1 * 16 + ax1]
                        with T.block("B_reindex_shared.dyn"):
                            v0, v1, v2 = T.axis.remap("SSS", [ax2_0_0, ax0_0_0_ax1_0_0_fused, ax0_0_1_ax1_0_1_fused])
                            T.reads(B[v0 * 16:v0 * 16 + 16, v1 % 2 * 1024 + v2 * 256:v1 % 2 * 1024 + v2 * 256 + 256])
                            T.writes(B_reindex_shared_dyn[v0 * 16:v0 * 16 + 16, v1 % 2 * 1024 + v2 * 256:v1 % 2 * 1024 + v2 * 256 + 256])
                            T.block_attr({"auto_copy": 1, "buffer_dim_align": [[0, 0, 32, 8]], "permuted_layout": "g2s_B", "vector_bytes": 16})
                            for ax0, ax1 in T.grid(16, 256):
                                B_reindex_shared_dyn[v0 * 16 + ax0, v1 % 2 * 1024 + v2 * 256 + ax1] = B[v0 * 16 + ax0, v1 % 2 * 1024 + v2 * 256 + ax1]
                        for ax2_0_1 in T.serial(2, annotations={"software_pipeline_order": [0, 1, 2], "software_pipeline_stage": [0, 0, 1]}):
                            for ax0_0, ax1_0 in T.grid(2, 1):
                                with T.block("A_reindex_shared.dyn_m16n8k8.matrixA_o"):
                                    v0_o = T.axis.spatial(64, ax0_0_0_ax1_0_0_fused // 2 * 4 + ax0_0_2_ax1_0_2_fused // 2 * 2 + ax0_0)
                            ...
                            for ax0_0, ax1_0 in T.grid(1, 4):
                                with T.block("B_reindex_shared.dyn_m16n8k8.matrixB_o"):
                                    ...
                                    v1_o = T.axis.spatial(64, ax0_0_0_ax1_0_0_fused % 2 * 32 + ax0_0_1_ax1_0_1_fused * 8 + ax0_0_2_ax1_0_2_fused % 2 * 4 + ax1_0)
                            ...
                            for ax0_0_3, ax1_0_3, ax2_0_2, ax0_0_4, ax1_0_4 in T.grid(1, 16, 1, 4, 1):
                                with T.block("Y_o_update"):
                                    v0_o = T.axis.spatial(128, ax0_0_0_ax1_0_0_fused // 2 * 8 + ax0_0_2_ax1_0_2_fused // 2 * 4 + ax0_0_3 * 4 + ax0_0_4)
                                    v1_o = T.axis.spatial(256, ax0_0_0_ax1_0_0_fused % 2 * 128 + ax0_0_1_ax1_0_1_fused * 32 + ax0_0_2_ax1_0_2_fused % 2 * 16 + ax1_0_3 + ax1_0_4)
                    ...
                    with T.block("Y_reindex_m16n8k8.matrixC"):
                        v0, v1, v2 = T.axis.remap("SSS", [ax0_0_0_ax1_0_0_fused, ax0_0_2_ax1_0_2_fused, ax0_0_1_ax1_0_1_fused])
                        T.reads(Y_reindex_m16n8k8_matrixC[v0 // 2 * 128 + v1 // 2 * 64:v0 // 2 * 128 + v1 // 2 * 64 + 64, v0 % 2 * 1024 + v2 * 256 + v1 % 2 * 128:v0 % 2 * 1024 + v2 * 256 + v1 % 2 * 128 + 128])
                        T.writes(Y[v0 // 2 * 128 + v1 // 2 * 64:v0 // 2 * 128 + v1 // 2 * 64 + 64, v0 % 2 * 1024 + v2 * 256 + v1 % 2 * 128:v0 % 2 * 1024 + v2 * 256 + v1 % 2 * 128 + 128])
                        T.block_attr({"auto_copy": 1})
                        for ax0, ax1 in T.grid(64, 128):
                            Y[v0 // 2 * 128 + v1 // 2 * 64 + ax0, v0 % 2 * 1024 + v2 * 256 + v1 % 2 * 128 + ax1] = Y_reindex_m16n8k8_matrixC[v0 // 2 * 128 + v1 // 2 * 64 + ax0, v0 % 2 * 1024 + v2 * 256 + v1 % 2 * 128 + ax1]
