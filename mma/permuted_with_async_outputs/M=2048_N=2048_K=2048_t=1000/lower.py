...
class Module:
    ...
    def main(A: T.Buffer((2048, 2048), "float16"), B: T.Buffer((2048, 2048), "float16"), Y: T.Buffer((2048, 2048), "float16")):
        ...
        for ax1_0_3_init, ax0_0_4_init, b in T.grid(16, 4, 2):
            Y_reindex_m16n8k8_matrixC_2[ax0_0_4_init * 32 + ax1_0_3_init * 2 + b] = T.Broadcast(T.float16(0), 2)
        with T.decl_buffer((6144,), "float16", data=A_reindex_shared_dyn, scope="shared.dyn") as A_reindex_shared_dyn_1:
            ...
            with T.attr(0, "async_commit_queue_scope", 0):
                with T.attr(0, "async_scope", 1):
                    for ax0_ax1_fused_0 in range(2):
                        A_reindex_shared_dyn_2[ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x * 8:ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x * 8 + 8] = A_1[blockIdx_x // 2 * 262144 + ax0_ax1_fused_0 * 131072 + threadIdx_y * 32768 + threadIdx_x // 2 * 2048 + threadIdx_x % 2 * 8:blockIdx_x // 2 * 262144 + ax0_ax1_fused_0 * 131072 + threadIdx_y * 32768 + threadIdx_x // 2 * 2048 + threadIdx_x % 2 * 8 + 8]
                T.attr(0, "async_scope", 1)
                for ax0_ax1_fused_0 in range(4):
                    B_reindex_shared_dyn_2[ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x // 8 * 64 + T.bitwise_xor(threadIdx_x % 8, ax0_ax1_fused_0 % 2 * 4 + threadIdx_y) * 8:ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x // 8 * 64 + T.bitwise_xor(threadIdx_x % 8, ax0_ax1_fused_0 % 2 * 4 + threadIdx_y) * 8 + 8] = B_1[ax0_ax1_fused_0 * 8192 + threadIdx_y * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + threadIdx_x * 8:ax0_ax1_fused_0 * 8192 + threadIdx_y * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + threadIdx_x * 8 + 8]
            with T.attr(0, "async_commit_queue_scope", 0):
                with T.attr(0, "async_scope", 1):
                    for ax0_ax1_fused_0 in range(2):
                        A_reindex_shared_dyn_2[ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x * 8 + 2048:ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x * 8 + 2048 + 8] = A_1[blockIdx_x // 2 * 262144 + ax0_ax1_fused_0 * 131072 + threadIdx_y * 32768 + threadIdx_x // 2 * 2048 + threadIdx_x % 2 * 8 + 16:blockIdx_x // 2 * 262144 + ax0_ax1_fused_0 * 131072 + threadIdx_y * 32768 + threadIdx_x // 2 * 2048 + threadIdx_x % 2 * 8 + 16 + 8]
                T.attr(0, "async_scope", 1)
                for ax0_ax1_fused_0 in range(4):
                    B_reindex_shared_dyn_2[ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x // 8 * 64 + T.bitwise_xor(threadIdx_x % 8, ax0_ax1_fused_0 % 2 * 4 + threadIdx_y) * 8 + 4096:ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x // 8 * 64 + T.bitwise_xor(threadIdx_x % 8, ax0_ax1_fused_0 % 2 * 4 + threadIdx_y) * 8 + 4096 + 8] = B_1[ax0_ax1_fused_0 * 8192 + threadIdx_y * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + threadIdx_x * 8 + 32768:ax0_ax1_fused_0 * 8192 + threadIdx_y * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + threadIdx_x * 8 + 32768 + 8]
            ...
            for ax2_0_0 in range(126):
                with T.attr(0, "async_commit_queue_scope", 0):
                    with T.attr(0, "async_scope", 1):
                        for ax0_ax1_fused_0 in range(2):
                            A_reindex_shared_dyn_2[(ax2_0_0 + 2) % 3 * 2048 + ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x * 8:(ax2_0_0 + 2) % 3 * 2048 + ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x * 8 + 8] = A_1[blockIdx_x // 2 * 262144 + ax0_ax1_fused_0 * 131072 + threadIdx_y * 32768 + threadIdx_x // 2 * 2048 + ax2_0_0 * 16 + threadIdx_x % 2 * 8 + 32:blockIdx_x // 2 * 262144 + ax0_ax1_fused_0 * 131072 + threadIdx_y * 32768 + threadIdx_x // 2 * 2048 + ax2_0_0 * 16 + threadIdx_x % 2 * 8 + 32 + 8]
                    T.attr(0, "async_scope", 1)
                    for ax0_ax1_fused_0 in range(4):
                        B_reindex_shared_dyn_2[(ax2_0_0 + 2) % 3 * 4096 + ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x // 8 * 64 + T.bitwise_xor(threadIdx_x % 8, ax0_ax1_fused_0 % 2 * 4 + threadIdx_y) * 8:(ax2_0_0 + 2) % 3 * 4096 + ax0_ax1_fused_0 * 1024 + threadIdx_y * 256 + threadIdx_x // 8 * 64 + T.bitwise_xor(threadIdx_x % 8, ax0_ax1_fused_0 % 2 * 4 + threadIdx_y) * 8 + 8] = B_1[ax2_0_0 * 32768 + ax0_ax1_fused_0 * 8192 + threadIdx_y * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + threadIdx_x * 8 + 65536:ax2_0_0 * 32768 + ax0_ax1_fused_0 * 8192 + threadIdx_y * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + threadIdx_x * 8 + 65536 + 8]
                with T.attr(0, "async_wait_queue_scope", 0):
                    T.attr(0, "async_wait_inflight_count", 1)
                    for ax0_0 in range(2):
                    ...
                    for ax1_0_3, ax0_0_4 in T.grid(16, 4):
            ...
            for ax1_0 in range(4):
                T.ptx_ldmatrix("float16", T.bool(True), 4, ".b16", B_reindex_shared_dyn_m16n8k8_matrixB, ax1_0 * 8, T.tvm_access_ptr(T.type_annotation("float16"), B_reindex_shared_dyn, 4096, 2048, 1), threadIdx_x % 8 * 256 + threadIdx_y % 2 * 128 + ax1_0 // 2 * 64 + T.bitwise_xor(ax1_0 % 2 * 4 + threadIdx_x // 8, threadIdx_x % 8) * 8)
            for ax1_0_3, ax0_0_4 in T.grid(16, 4):
            ...
            for ax1_0_3, ax0_0_4 in T.grid(16, 4):
                ...
                T.ptx_mma("float16", "m16n8k8", "row", "col", "fp16", "fp16", "fp16", A_reindex_shared_dyn_m16n8k8_matrixA, ax0_0_4 * 4, B_reindex_shared_dyn_m16n8k8_matrixB, cse_var_5, Y_reindex_m16n8k8_matrixC, ax0_0_4 * 32 + cse_var_5, T.bool(False))
            for ax1_0_3, ax0_0_4 in T.grid(16, 4):
                cse_var_6: T.int32 = ax1_0_3 * 2
                T.ptx_mma("float16", "m16n8k8", "row", "col", "fp16", "fp16", "fp16", A_reindex_shared_dyn_m16n8k8_matrixA, ax0_0_4 * 4 + 16, B_reindex_shared_dyn_m16n8k8_matrixB, cse_var_6 + 32, Y_reindex_m16n8k8_matrixC, ax0_0_4 * 32 + cse_var_6, T.bool(False))
        ...
        for ax0_0 in range(8):
            ...
            for ax0_0_2_ax1_0_2_fused_cache_ax1_0_cache_ax0_1_cache_ax1_1_cache_fused_0 in range(32):
                ...
                Y_1[blockIdx_x // 2 * 262144 + ax0_0_2_ax1_0_2_fused_cache_ax1_0_cache_ax0_1_cache_ax1_1_cache_fused_0 // 16 * 131072 + ax0_0 * 16384 + threadIdx_y % 2 * 8192 + threadIdx_x // 8 * 2048 + blockIdx_x % 2 * 1024 + blockIdx_y * 256 + ax0_0_2_ax1_0_2_fused_cache_ax1_0_cache_ax0_1_cache_ax1_1_cache_fused_0 % 16 * 16 + threadIdx_y // 2 * 8 + threadIdx_x % 8] = A_reindex_shared_dyn_1[ax0_0_2_ax1_0_2_fused_cache_ax1_0_cache_ax0_1_cache_ax1_1_cache_fused_0 * 128 + threadIdx_y * 32 + threadIdx_x]
