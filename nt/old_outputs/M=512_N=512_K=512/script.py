            ...
            for i_1_j_1_fused in T.thread_binding(2, thread="vthread.x"):
                for i_2_j_2_fused in T.thread_binding(128, thread="threadIdx.x"):
                    for i_3_init, j_3_init, i_4_init, j_4_init in T.grid(1, 2, 4, 1):
                        with T.block("Y_init"):
                            v_i = T.axis.spatial(512, i_0_j_0_fused // 16 * 64 + i_2_j_2_fused // 8 * 4 + i_3_init * 4 + i_4_init)
                            v_j = T.axis.spatial(512, j_4_init + i_0_j_0_fused % 16 * 32 + i_1_j_1_fused * 16 + i_2_j_2_fused % 8 * 2 + j_3_init)
                            T.reads()
                            ...
                            for ax0_ax1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_fused_2 in T.vectorized(4):
                                    with T.block("A_shared"):
                                        ...
                                        v1 = T.axis.spatial(512, i_0_j_0_fused // 16 * 64 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) % 64)
                                        T.reads(A[v0, v1])
                            ...
                            for ax0_ax1_fused_1 in T.thread_binding(128, thread="threadIdx.x"):
                                for ax0_ax1_fused_2 in T.vectorized(4):
                                    with T.block("B_shared"):
                                        v0 = T.axis.spatial(512, k_0 * 64 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) // 32)
                                        v1 = T.axis.spatial(512, i_0_j_0_fused % 16 * 32 + (ax0_ax1_fused_0 * 512 + ax0_ax1_fused_1 * 4 + ax0_ax1_fused_2) % 32)
                                        T.reads(B[v0, v1])
                                        T.writes(B_shared[v0, v1])
                                        B_shared[v0, v1] = B[v0, v1]
                        for k_1, i_3, j_3, k_2, i_4, j_4 in T.grid(16, 1, 2, 4, 4, 1):
                            with T.block("Y_update"):
                                v_i = T.axis.spatial(512, i_0_j_0_fused // 16 * 64 + i_2_j_2_fused // 8 * 4 + i_3 * 4 + i_4)
                                v_j = T.axis.spatial(512, j_4 + i_0_j_0_fused % 16 * 32 + i_1_j_1_fused * 16 + i_2_j_2_fused % 8 * 2 + j_3)
                                v_k = T.axis.reduce(512, k_0 * 64 + k_1 * 4 + k_2)
                    ...
                    for ax0, ax1 in T.grid(4, 2):
                        with T.block("Y_local"):
                            v0 = T.axis.spatial(512, i_0_j_0_fused // 16 * 64 + i_2_j_2_fused // 8 * 4 + ax0)
                            v1 = T.axis.spatial(512, i_0_j_0_fused % 16 * 32 + i_1_j_1_fused * 16 + i_2_j_2_fused % 8 * 2 + ax1)
