...
def stochastic_schedule_regular(sch: tvm.tir.Schedule):
    ...
    sch.transform_layout(A_gmem2smem, buffer=("write", 0), index_map=lambda i, j: (j, i))
    ...
    vwarp_idx_m, vlane_offset_m_major, vlane_offset_m_minor = sch.sample_perfect_tile(loop=temp_tx, n=3, max_innermost_factor=2)
    ...
    ax0 = sch.get_loops(block=A_smem2local)[-2]
    ...
    vtemp_ax0A, vjA = sch.sample_perfect_tile(loop=ax0A, n=2)
    temp_ax0A, jA = sch.split(loop=ax0A, factors=[vtemp_ax0A, vjA])
    sch.reorder(temp_ax0A, ax1A, jA)
    txA = sch.fuse(temp_ax0A, ax1A)
...
try:
    ...
    with open('finished_task.txt', 'a') as f:
        f.write(f'\n[FINISH | stochastic] M={M} N={N} K={K}')
except:
    with open('finished_task.txt', 'a') as f:
        f.write(f'\n[FAIL | stochastic] M={M} N={N} K={K}')
