...
@register_object("meta_schedule.MultiLevelTilingPipelined")
class MultiLevelTilingPipelined(ScheduleRule):
    """Extension of MultiLevelTiling with software pipeline and 
    multiple-level reuse read.

    Parameters
    ----------
    structure : str
        The tiling structure. Current only support:
        - 'SSRRS' on GPU
    tile_bind : Optional[List[str]]
        For each level of tiles, which thread axis it is bound to.
        Current only support:
        - [blockIdx.x, threadIdx.x] on GPU
    max_innermost_factor : Optional[int]
        The maximum size of the innermost factor. None means no limit
    vector_load_lens : Optional[List[int]]
        The length of vector lane in vectorized cooperative fetching.
        None means disable vectorization
    reuse_read : Optional[List[ReuseType]]
        Array of data reuse configuration for reading. Multiple config
        will be run sequentially. None means no reuse.
    reuse_write : Optional[ReuseType]
        Data reuse configuration for writing. None means no reuse.
    """
    ...
    def __init__(
        ...
        max_innermost_factor: Optional[int] = None,
        vector_load_lens: Optional[List[int]] = None,
        reuse_read: Optional[List[ReuseType]] = None,
        reuse_write: Optional[ReuseType] = None,
    ) -> None:
        self.__init_handle_by_constructor__(
            _ffi_api.ScheduleRuleMultiLevelTilingPipelined,  # type: ignore # pylint: disable=no-member
            structure,
            tile_binds,
            max_innermost_factor,
            vector_load_lens,
            [r.as_dict() for r in reuse_read] if reuse_read is not None else None,
            reuse_write.as_dict() if reuse_write is not None else None,
        )
