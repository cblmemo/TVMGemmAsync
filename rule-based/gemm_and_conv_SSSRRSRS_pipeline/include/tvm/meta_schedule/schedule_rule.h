...
class ScheduleRule : public runtime::ObjectRef {
 public:
  ...
  /*!
   * \brief Extension of MultiLevelTiling with software pipeline and multiple-level reuse read
   * \param structure The tiling structure. Current only support:
   * - 'SSRRS' on GPU
   * \param tile_binds For each level of tiles, which thread axis it is bound to.
   * Current only support:
   * - [blockIdx.x, threadIdx.x] on GPU
   * \param max_innermost_factor The maximum size of the innermost factor. NullOpt means no limit
   * \param vector_load_lens The length of vector lane in vectorized cooperative fetching.
   * NullOpt means disable vectorization
   * \param reuse_read Array of data reuse configuration for reading. Multiple config will be run sequentially.
   * NullOpt means no reuse.
   * \param reuse_write Data reuse configuration for writing. NullOpt means no reuse.
   * \return The schedule rule created
   */
  TVM_DLL static ScheduleRule MultiLevelTilingPipelined(String structure,                                    //
                                                        Optional<Array<String>> tile_binds,                  //
                                                        Optional<Integer> max_innermost_factor,              //
